...
# Eager: `/models/{name}:method` paths overlap with the OpenAI `/models`
# list endpoint, so a `/models` prefix can't safely gate this lazily.
app.include_router(google_router)

# Attach lazy-feature middleware: imports + registers each optional
# router on the first request that hits its path prefix. See _lazy_features.py.
